package android.leo.electricityboardapplication;
...
public class ElectricityBoardView extends View {
    ...
    private int mQuantity = mMin; // 实时电量
    ...
    private void init() {
        ...
        mTexts = new String[]{"0", "150", "210", "350", "400", "1000"};
    ...
    protected void onDraw(Canvas canvas) {
         ...
         * 画指针
         */
        float θ = mStartAngle + mSweepAngle * (mQuantity - mMin) / (mMax - mMin); // 指针与水平线夹角
        ...
        if (mQuantity >= 100) {
            drawDigitalTube(canvas, mQuantity / 100, -xOffset);
            drawDigitalTube(canvas, (mQuantity - 100) / 10, 0);
            drawDigitalTube(canvas, mQuantity % 100 % 10, xOffset);
        } else if (mQuantity >= 10) {
            drawDigitalTube(canvas, -1, -xOffset);
            drawDigitalTube(canvas, mQuantity / 10, 0);
            drawDigitalTube(canvas, mQuantity % 10, xOffset);
        } else {
            drawDigitalTube(canvas, -1, -xOffset);
            drawDigitalTube(canvas, -1, 0);
            drawDigitalTube(canvas, mQuantity, xOffset);
        }
